    ...
    def sequence_imgs(self, abs_path, passage_path,  direction):
        ...
        for nom_image in images_depth:
            ...
            if is_image_valid(d_img):
                ...
                rgb_img = cv2.imread(os.path.join(abs_path, passage_path, nom_image.replace("_D", "_RGB")), cv2.IMREAD_UNCHANGED)
...
class SequenceEncoder(L.LightningModule):
    def __init__(self, embedding_size, NbFrames):
        ...
        self.positional_encoder = LearnedSeqEncoding(d_model = embedding_size, seq_len=NbFrames)
        encoder_layer_local = nn.TransformerEncoderLayer(d_model=embedding_size,
                                                   nhead=8,
        ...
        self.loss = HardBatchedTripletLoss(margin=1.2)
    ...
    def forward(self, depth_sequence):
        ...
        return output, torch.mean(depth_encoded, dim=0)

    def training_step(self, batch, batch_idx):

        anchor_depth_sequence, postive_depth_sequence, negative_depth_sequence, idx = batch
        ...
        anchor_out, depth_anchor = self.forward(anchor_depth_sequence.view(-1, N, 1, H, W))
        positive_out, depth_positive = self.forward(postive_depth_sequence.view(-1, N, 1, H, W))
        negative_out, depth_negative = self.forward(negative_depth_sequence.view(-1, N, 1, H, W))
        ...
        total_loss = train_loss + depth_loss
