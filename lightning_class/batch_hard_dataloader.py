...
class HardBatchedTripletLoss(nn.Module):
    ...
    def forward(self, anchor: torch.Tensor, positive: torch.Tensor, negative: torch.Tensor) -> torch.Tensor:
        ...
        distance_positive, _ = self.cosine_distance_matrix_batch(anchor, positive).max(1, keepdim=True)
        # print("distance_positive", distance_positive)
        distance_negative, _ = self.cosine_distance_matrix_batch(anchor, negative.view(-1, hidden_size).expand(B, -1, -1)).min(1, keepdim=True)
...
class SequenceEncoder(L.LightningModule):
    def __init__(self, embedding_size, NbFrames, Nb_layerToFreeze=4):
        ...
        self.loss = HardBatchedTripletLoss(margin=0.5)
    ...
    def training_step(self, batch, batch_idx):
        ...
        anchor_out = self.forward(anchor_depth_sequence.view(-1, N, 1, H, W), anchor_rgb_sequence.view(-1, N, 3, H, W))
        positive_out = self.forward(postive_depth_sequence.view(-1, N, 1, H, W), postive_rgb_sequence.view(-1, N, 3, H, W))
        negative_out = self.forward(negative_depth_sequence.view(-1, N, 1, H, W), negative_rgb_sequence.view(-1, N, 3, H, W))
